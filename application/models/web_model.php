...
class Web_model extends CI_Model {
	...
	function get_pengguna_by_id($id){
		...
		$rs = $this->db->query("SELECT * FROM pengguna WHERE id = $id");
		return $rs;
		
	}
	
	
	function get_surat_keluar_by_id($id){
		$rs = $this->db->query("SELECT * FROM surat_keluar WHERE id = $id");
		return $rs;
	}
	
	
	function is_pemeriksa_hadir($id_surat){
		$rs = $this->db->query("SELECT b.kehadiran_status as status_kehadiran FROM surat_keluar a ".
							   "LEFT JOIN pengguna b ON a.pemeriksa_user = b.id ".
							   "WHERE a.id = $id_surat");
	    return $rs->row()->status_kehadiran === 'hadir' ? true : false;
